from csql import Parameters, Q, Query, RenderedQuery
	...
	class MySQLRenderer(QueryRenderer):
		def _render(self, query: Query) -> RenderedQuery:
			...
			return RenderedQuery(
				sql="hello hello", parameters=values, parameter_names=names
			)

	p = Parameters(abc="abc")
	q = Q(
		f"select 1 where abc = {p['abc']}",
		overrides=Overrides(queryRenderer=MySQLRenderer),
	)
